...
fun <P, T> Async.Companion.actions(
    async: SourcedSetter<*, Async<T>>,
    ...
    Actions(
        scope,
        pipe,
        onCancel = { async.set(failure(it)) },
        onReset = { async.modify { if (it.isSuccess or it.isFailure) idle() else it } },
        onExecute = { output ->
            output
                .let(behavior)
                .onEach(async::set)
...
fun <P, T> SourcedSetter<*, Async<T>>.toActions(
    ...
    Async.actions(this, behavior, scope, effect, MutableSharedFlow<P>().toPipe())